...
async function unlockPreview(page: import('@playwright/test').Page) {
  await page.goto('/')
  await expect(page.getByRole('heading', { name: 'Войти в аккаунт', exact: true })).toBeVisible({ timeout: 15_000 })
...
test('unified search, messages and gift modes have real behavior', async ({ page }) => {
  ...
  let search = page.locator('.hero-search').getByRole('textbox')
  await search.fill('desktop')
  await expect(page.getByRole('tab', { name: /^Посты/ })).toBeVisible()
  await page.getByRole('tab', { name: /^Посты/ }).click()
  const postResult = page.locator('.search-results section').filter({ has: page.getByRole('heading', { name: 'Публикации' }) }).getByRole('button', { name: /mummy/i }).first()
  await expect(postResult).toBeVisible()
  ...
  await page.locator('.app-rail').getByRole('button', { name: 'Поиск', exact: true }).click()
  search = page.locator('.hero-search').getByRole('textbox')
...
test('native shell switches to the compact Messenger Dark workspace', async ({ page }) => {
  ...
  await expect(page.getByRole('tab', { name: 'Сообщения', exact: true })).toHaveAttribute('aria-selected', 'true')
  await page.getByRole('tab', { name: 'Обзор', exact: true }).click()
  await expect(page.locator('.workspace-head h1')).toContainText('Уведомления')
  await page.locator('.app-rail').getByRole('button', { name: 'Сообщения', exact: true }).click()
  const conversations = page.locator('.conversation-list [data-conversation-item="true"]')
  await conversations.first().focus()
  await page.keyboard.press('ArrowDown')
  await expect(conversations.nth(1)).toBeFocused()
  await page.keyboard.press('Enter')
  await expect(conversations.nth(1)).toHaveClass(/is-active/)
...
test('desktop panels resize with keyboard and the profile media tab is interactive', async ({ page }) => {
  ...
  await expect(page.getByRole('tab', { name: 'Подарки', exact: true })).toBeVisible()
  await page.getByRole('tab', { name: 'Подарки', exact: true }).click()
  await expect(page.getByRole('tab', { name: 'Подарки', exact: true })).toHaveAttribute('aria-selected', 'true')
  await expect(page.getByRole('tabpanel').getByRole('button', { name: 'Открыть подарки аккаунта' })).toBeVisible()
...
test('main desktop destinations have no serious automated accessibility violations', async ({ page }) => {
  ...
  await unlockPreview(page)
  for (const destination of ['Лента', 'Поиск', 'Уведомления', 'Сообщения', 'Сообщества', 'Студия', 'Подарки UnixGram', 'Профиль', 'Unix Premium', 'Настройки клиента']) {
...
test('native client settings expose themes, chat controls and window controls', async ({ page }) => {
  ...
  await expect(page.getByText('Компактный текст')).toBeVisible()
  await expect(page.getByText('Полный экран')).toBeVisible()
  await expect(page.getByText('Масштаб')).toBeVisible()
  await expect(page.getByText('Счётчик в трее')).toBeVisible()
  await expect(page.getByText('Уведомления Windows')).toBeVisible()
  await expect(page.getByText('Переподключаться при обрыве')).toBeVisible()
  await expect(page.getByText('Глобальные сочетания')).toBeVisible()
  await expect(page.getByText('Центр безопасности')).toBeVisible()
  await expect(page.getByText('Разрешённые сайты')).toBeVisible()
  await expect(page.getByRole('heading', { name: 'Аккаунты' })).toBeVisible()
  await expect(page.getByText('До трёх аккаунтов')).toHaveCount(1)
  await expect(page.getByRole('checkbox', { name: /Счётчик в трее/ })).toBeChecked()
  await expect(page.getByRole('checkbox', { name: /Уведомления Windows/ })).toBeChecked()
  await expect(page.getByRole('checkbox', { name: /Переподключаться при обрыве/ })).toBeChecked()
  await expect(page.getByRole('checkbox', { name: /Глобальные сочетания/ })).toBeChecked()
  const shortcuts = page.getByLabel('Сочетания клавиш')
  await expect(shortcuts).toContainText('UnixPlace')
  await expect(shortcuts).toContainText('CtrlShift1')
  await expect(shortcuts).toContainText('CtrlShift4')
  await expect(page.getByText(/Ctrl \+ Alt \+ U\/M\/G\/S сохранены/)).toBeVisible()
  await expect(page.getByRole('button', { name: 'Сменить основной аккаунт' })).toBeVisible()
  await expect(page.getByRole('button', { name: /Читать @basaltes/ })).toBeVisible()
  await expect(page.getByRole('button', { name: /Добавить/ })).toBeDisabled()
  await expect(page.getByText('cookie и пароли не записываются')).toHaveCount(1)
  await expect(page.locator('.theme-card')).toHaveCount(12)
})

test('both account settings views allow a blank second slot and expose primary reset', async ({ page }) => {
  await unlockPreview(page)
  await page.getByRole('button', { name: 'Настройки клиента' }).click()
  await page.getByLabel('Разделы настроек').getByRole('button', { name: 'Аккаунты' }).click()
  const embeddedAdd = page.getByRole('button', { name: 'добавить аккаунт' })
  await expect(page.getByLabel('Подпись аккаунта')).toHaveValue('')
  await expect(embeddedAdd).toBeEnabled()
  await expect(page.getByRole('button', { name: 'сменить аккаунт' })).toBeVisible()
...
test('standalone settings page has named controls and no serious accessibility violations', async ({ page }) => {
  ...
  await page.goto('/?desktop-settings=1')
  await expect(page.getByRole('heading', { name: 'Настройки' })).toBeVisible()
  await expect(page.locator('header').getByRole('button', { name: 'Предпросмотр', exact: true })).toBeDisabled()
  await expect(page.locator('footer').getByRole('button', { name: 'Предпросмотр', exact: true })).toBeDisabled()
  await expect(page.getByRole('checkbox', { name: 'Liquid Glass', exact: true })).toBeVisible()
  await expect(page.getByRole('checkbox', { name: 'Компактный текст', exact: true })).toBeVisible()
...
test('standalone settings page should scroll to the footer on shorter viewports', async ({ page }) => {
  ...
  const footerButton = page.locator('footer').getByRole('button', { name: 'Предпросмотр', exact: true })

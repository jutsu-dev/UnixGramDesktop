...
test('native client settings expose themes, chat controls and window controls', async ({ page }) => {
  ...
  await expect(page.getByText('Непрочитанные в трее')).toBeVisible()
  await expect(page.getByText('Уведомления Windows')).toBeVisible()
  await expect(page.getByText('Восстанавливать соединение')).toBeVisible()
  ...
  await expect(page.getByText('до трёх отдельных сессий UnixGram')).toHaveCount(1)
  await expect(page.getByRole('checkbox', { name: /Непрочитанные в трее/ })).toBeChecked()
  await expect(page.getByRole('checkbox', { name: /Уведомления Windows/ })).toBeChecked()
  await expect(page.getByRole('checkbox', { name: /Восстанавливать соединение/ })).toBeChecked()
